...
long long verificaPrimo(long long p){
	...
	j = sqrt(p);//Calcula a raiz quadrada para p
...
long potencia(long long a, long long e, long long n){
	...
	while(1){
		...
		if(E == 0){
			return P; //Chegou ao fim da expansão, retorna o P
		}else{
			if(E%2 != 0){ //Se o expoente é ímpar
...
int main(void){
	...
	char mensagem[100]; //A mensagem a ser criptografada pode ter no máximo 100 caracteres
	int *mensagemC;
	long long primoFlag;//Verifica se é um primo
	...
	switch (opc){
		case 1:
			...
			for(i = 0; i < strlen(mensagem); i++){
				printf("%i", mensagemC[i]);
			}
			printf("\n");
			break;
		case 2:
			//Lê um primo do teclado
			printf("\nDigte o primeiro número\n");
			scanf("%llu", &p);
			...
			char *mensagemD;
			mensagemD = decodifica(mensagemC, p, p2);
