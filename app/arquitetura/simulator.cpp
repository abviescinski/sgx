...
int main(int argc, char **argv) {
	...
	// aloca config cache
    orcs_engine.cache = new cache_t[NIVEL_CACHE];
    for(size_t i = 0; i<NIVEL_CACHE;i++){
    ...
    //apresenta as informações obtidas (acertos e erros) de cada cache
	for(size_t i = 0; i<NIVEL_CACHE;i++){
		orcs_engine.cache[i].estatisticas();
	}
